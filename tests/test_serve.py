...
def _mock_cfg(mocker, service_name="vllm", active_yaml=None):
    ...
    mocker.patch("vlx.serve.cfg", return_value=mock)
    return mock


def test_is_vllm_running_active(mocker):
    _mock_cfg(mocker)
    mocker.patch("vlx.serve._systemctl", return_value=(True, "active", ""))
    assert is_vllm_running() is True


def test_is_vllm_running_inactive(mocker):
    _mock_cfg(mocker)
    mocker.patch("vlx.serve._systemctl", return_value=(True, "inactive", ""))
    assert is_vllm_running() is False


def test_stop_vllm(mocker):
    _mock_cfg(mocker)
    mock_run = mocker.patch("vlx.serve._systemctl", return_value=(True, "", ""))
    stop_vllm()
...
def test_start_vllm_with_config(mocker, tmp_path):
    ...
    mock_systemctl = mocker.patch("vlx.serve._systemctl", return_value=(True, "", ""))
...
def test_systemctl_uses_service_name(mocker):
    ...
    mock_run = mocker.patch("vlx.serve.subprocess.run", return_value=Mock(returncode=0, stdout="active", stderr=""))
    is_vllm_running()
...
def test_start_vllm_failure(mocker, tmp_path):
    ...
    active = tmp_path / "active.yaml"
    _mock_cfg(mocker, active_yaml=active)
    mocker.patch("vlx.serve._systemctl", return_value=(False, "", "Unit not found"))
...
def test_stop_vllm_failure(mocker):
    ...
    _mock_cfg(mocker)
    mocker.patch("vlx.serve._systemctl", return_value=(False, "", "Failed to stop"))
